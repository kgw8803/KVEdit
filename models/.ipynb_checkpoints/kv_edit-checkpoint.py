...
class Flux_kv_edit(only_Flux):
    ...
    def denoise(self,z0,zt,inp_target,mask:Tensor,opts,info):
        print("🚨 [DEBUG] z_bg shape:", z0.shape)
        print("🚨 [DEBUG] z_fg shape:", zt.shape)
        print("🚨 [DEBUG] mask shape:", mask.shape, "dtype:", mask.dtype, "max:", mask.max().item(), "min:", mask.min().item())

        # zt_noise가 생성된 후 시점이라면
        try:
            print("🚨 [DEBUG] zt_noise shape:", zt_noise.shape)
        except:
            print("🚨 [DEBUG] zt_noise not defined yet")
        ...
        # ✅ 여기에서 mask_indices에 대한 실제 디버깅 로그 삽입
        print("🧩 [DEBUG] bool_mask shape:", bool_mask.shape)
        print("🧩 [DEBUG] mask_indices:", info["mask_indices"].shape)
        print("🧩 [DEBUG] mask_indices max:", info["mask_indices"].max().item() if info["mask_indices"].numel() > 0 else "EMPTY")
        ...
        # ✅ 최종 indexing 전에 디버깅
        print("🧩 [DEBUG] z0 shape:", z0.shape)
        print("🧩 [DEBUG] x shape:", x.shape)
        print("🧩 [DEBUG] mask shape:", info['mask'].shape)
        print("🧩 [DEBUG] mask_indices shape:", mask_indices.shape)
        print("🧩 [DEBUG] mask_indices max:", mask_indices.max().item() if mask_indices.numel() > 0 else "EMPTY")
        try:
            print("🧩 [DEBUG] z0[:, mask_indices, ...] shape:", z0[:, mask_indices, ...].shape)
            print("🧩 [DEBUG] info['mask'][:, mask_indices, ...] shape:", info['mask'][:, mask_indices, ...].shape)
        except Exception as e:
            print("❌ [ERROR] Indexing shape mismatch:", str(e))

                # 구체적 차원도 보기
        print("🧩 [DEBUG] z0.ndim:", z0.ndim)
        print("🧩 [DEBUG] x.ndim:", x.ndim)
        print("🧩 [DEBUG] mask.ndim:", info['mask'].ndim)
